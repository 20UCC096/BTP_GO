...
class CFGListener(GoParserListener):
	...
	def new_node(self, t, content="", shape='box'):
		...
		if t == 'b':
			if self.nodes and self.if_stack:
				if_level_ok = self.if_levels_stack and self.if_levels_stack[-1] == self.level 

				if self.nodes[-1][0] == 'i':
					t = 't'

				elif if_level_ok and self.if_stack[-1] == If.ThenElse and len(self.lasts_stack)>=2 and self.lasts_stack[-2][0] == 'i': # TODO refactor
					t = 'e'
		...
		if self.lasts_stack:
			...
			if_level_ok = self.if_levels_stack and self.if_levels_stack[-1] == self.level 
			is_then = lasti[0] == 'i'
			is_else = if_level_ok and self.if_stack[-1] == If.ThenElse and len(self.ends)>=2 and self.ends[-2][0] == 'i'
			for_level_ok = self.for_level_stack and self.for_level_stack[-1] == self.level
			loop = for_level_ok and lasti[0] == 'l'

			if not (is_then or is_else or loop):
				last = self.ends.pop()
	...
	def connect(self, fr, to, manual= False, **kwargs):
		if type(fr) == list:
			for f in fr:
				self.connect(f, to, manual=manual)
		else:
			self.graph.edge(
					style(fr, 's'), 
					style(to, 'n'), **kwargs)
		...
		if not manual:
			if not self.ends:
				...
				if (type(last) == list and to not in last) or (to!=last) and to[0] != 'd':
					self.ends.append(to)
	...
	def enterIfStmt(self, ctx):
		...
		node = self.new_node('if', ctx.getChild(1).getText(), shape='diamond')
		self.firsts_stack.add(node)
	...
	def exitIfStmt(self, ctx):
		if ctx.getChildCount() == 5:
			...
			ifnode = self.firsts_stack.pop()
			thennodebegin = self.advance(ifnode)
	...
	def advance(self, nodes):
		...
		ind = self.nodes.index(nodes.split(':')[0])
	...
	def enterForStmt(self, ctx):
		...
		self.new_node('l', 'for '+ctx.getChild(1).getText(), shape='egg')

		self.for_level_stack.append(self.ind)
		self.breaks.append([])

	def exitForStmt(self, ctx):
		self.for_level_stack.pop()
		...
		if type(loopnodeend) == list:
			self.ind += 1
			dummy = f'd{self.ind}'
			self.connect(loopnodeend, dummy, manual=True)
			self.connect(dummy, loopnode, manual=True, headport='w', tailport='w')
		else:
			self.connect(loopnodeend, loopnode, manual=True, headport='w', tailport='w')

		self.ends.pop()
		self.ends.append(f'{loopnode}:e')

		self.graph.render()
		self.lasts_stack.add(f'{loopnode}:e')
		self.firsts_stack.add(f'{loopnode}')